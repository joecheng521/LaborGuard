...
def extract_laws_from_text(text):
    try:
        ...
        article_pattern = re.compile(r'第([零一二三四五六七八九十百]+)\s*条\s*(.*?)$')

        # 匹配章节和节标题
        chapter_pattern = re.compile(r'第[零一二三四五六七八九十百]+章\s*.*')
        section_pattern = re.compile(r'第[节一二三四五六七八九十]+节\s*.*')

        # 匹配条款中的分项
        subitem_pattern = re.compile(r'（\s*[一二三四五六七八九十]\s*）')

        # 使用有序字典存储条款
        article_dict = OrderedDict()
        current_article_num = None
        current_content = ""
        in_special_chapter = False  # 用于标记特别章节
        ...
        for i, line in enumerate(lines):
            ...
            if chapter_pattern.match(line):
                in_special_chapter = "特别规定" in line
                continue
        ...
        for article_num, content in article_dict.items():
            key = f"中华人民共和国劳动合同法 {article_num}"
            laws.append({key: content})

        # 确保包含所有必要条款（特别是第九十八条）
        if "第九十八条" not in article_dict:
            laws.append({"中华人民共和国劳动合同法 第九十八条": "本法自2008年1月1日起施行。"})
...
def validate_laws_data(laws_data, expected_count):
    ...
    # 检查缺失条款
    all_articles = {f"第{number_to_chinese(i)}条" for i in range(1, 99)}
    extracted_articles = {list(art.keys())[0].split()[-1] for art in laws_data}
    missing = sorted(all_articles - extracted_articles, key=lambda x: chinese_to_number(x[1:-2]))
    ...
    # 检查重要条款内容
    important_articles = {
        "第四十一条": ["需要裁减人员二十人以上", "优先留用下列人员", "在六个月内重新招用人员"],
        "第五十七条": ["经营劳务派遣业务应当具备下列条件", "申请行政许可", "公司登记"],
...
if __name__ == "__main__":
    # 配置路径
    input_docx = '../../old/中华人民共和国劳动合同法.docx'
    output_json = '../../data/中华人民共和国劳动合同法.json'
    EXPECTED_ARTICLE_COUNT = 98
    ...
    try:
        ...
        # 打印样本
        print("\n样本条款:")
        samples = [laws_data[0], laws_data[40], laws_data[56], laws_data[-1]]
        for sample in samples:
            key = list(sample.keys())[0]
            print(f"{key} ({len(sample[key])}字符):")
            print(sample[key][:150] + "..." if len(sample[key]) > 150 else sample[key])
            print("-" * 80)
